...
require_once("$CFG->dirroot/mod/url/classes/external.php");
...
function url_print_workaround($url, $cm, $course) {
    ...
    $userMaxScore = mod_url_external::get_user_max_score($USER->id)['max_score'];

    if ($userMaxScore != -1) {
        echo '<h4>Your Max. Score: <b>' . $userMaxScore . '</b></h4>';
        echo 'Keep playing to improve your ranking 🎖️';
    } else {
        echo '<h4>You did not register a score, play now!</h4>';
        echo 'Just click the link below 👇';
    }

    echo '<div style="padding: 25px;"/>';
    
    // Generate JWT
    $jwtSecret = 'your_secret_key'; // Replace with your actual secret key
    $userData = array(
        'username' => $USER->username,
        'id' => $USER->id
    );
    $jwt = JWT::encode(json_encode($userData), $jwtSecret);

    echo '<div class="urlworkaround">';
    print_string('clicktoopen', 'url', "<a href=\"$fullurl?token=$jwt\" $extra>$fullurl</a>");
    echo '</div>';
