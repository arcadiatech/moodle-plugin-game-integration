...
class mod_url_external extends external_api {
     ...
     * @since Moodle 3.0
     */
    public static function save_score_parameters() {
        return new external_function_parameters(
            array(
     ...
     * @throws moodle_exception
     */
    public static function save_score($userid, $score) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/url/lib.php");

        $params = self::validate_parameters(self::save_score_parameters(), array(
            'userid' => $userid,
            'score' => $score
        ));
        $warnings = array();
        ...
        $record_id = $DB->insert_record('score', $data);
     ...
     * @since Moodle 3.0
     */
    public static function save_score_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'status: true if success'),
                'warnings' => new external_warnings()
     ...
     * @since Moodle 3.0
     */
    public static function get_user_max_score_parameters() {
        return new external_function_parameters(
            array(
                'userid' => new external_value(PARAM_INT, 'userid')
            )
     ...
     * @throws moodle_exception
     */
    public static function get_user_max_score($userid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/url/lib.php");

        $params = self::validate_parameters(self::get_user_max_score_parameters(), array('userid' => $userid));
        ...
        $sql = "SELECT * FROM {score} WHERE user = :userid ORDER BY score DESC LIMIT 1";
        $sqlparams = ['userid' => $userid];
        $record = $DB->get_record_sql($sql, $sqlparams);

        if ($record) {
            $result['max_score'] = $record->score;
            $result['status'] = true;
        }
     ...
     * @since Moodle 3.0
     */
    public static function get_user_max_score_returns() {
        return new external_single_structure(
            array(
                'max_score' => new external_value(PARAM_INT, 'max score'),
                'status' => new external_value(PARAM_BOOL, 'status: true if success'),
                'warnings' => new external_warnings()
            )
        );
